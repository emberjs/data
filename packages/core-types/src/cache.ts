...
import type { ResourceDocument, SingleResourceDataDocument, V3ResourceDocument } from './spec/document';
...
export interface CacheV2 {
   ...
   *
   * This does not guarantee release of any associated resources unless the cache determines
   ...
   * @param {StableDocumentIdentifier} identifier
   * @return {boolean} whether a request was removed
   */
  removeRequest(identifier: StableDocumentIdentifier): boolean;
 ...
 * data.
 *
 * @class <Interface> CacheV3
 * @public
 */
export interface CacheV3 {
  /**
   * The Cache Version that this implementation implements.
   *
   * @type {'3'}
   ...
   * @method put
   * @param {StructuredDocument} doc
   * @return {V3ResourceDocument}
   * @public
   */
  put<T>(doc: StructuredDocument<T> | { content: T }): V3ResourceDocument;
  ...
  /**
   * Peek resource or document data from the Cache.
   *
   * In V3, this method should return remote state
   * (no mutations applied). In V2 it was not specified
   * whether the return should or should not include
   * mutated state, and the JSONPAPICache V2 implementation
   * chose to return mutated state.
   *
   * The V2 peek behavior is replaced by `getResource`
   *
   * In development, if the return value
   * is JSON the return value
   * will be deep-cloned and deep-frozen
   ...
   * Added in V3
   *
   * Peek the state of a resource from the cache with any
   * mutations applied.
   *
   * In V2 it was not specified whether the return of peek should
   * or should not include mutated state, and the JSONPAPICache V2
   * implementation chose to return mutated state. This method
   ...
   * @public
   * @param {StableRecordIdentifier} identifier
   * @return {ResourceBlob | null} the known resource data
   */
  getResource<T = unknown>(identifier: StableRecordIdentifier<TypeFromInstanceOrString<T>>): T | null;
   ...
   * @return {unknown}
   */
  peekAttr(identifier: StableRecordIdentifier, field: string | string[]): Value | undefined;

  /**
   * Retrieve the data for an attribute from the cache, with any
   * mutations applied
   *
   * @method getAttr
   * @public
  ...
  /**
   * Query the cache for the changes to a specific attribute of a resource.
   *
   * Returns a tuple of [old, new] values, or `null` if no change exists
   *
   * ```
   * [<old>, <new>]
   * ```
   ...
   * @param identifier
   * @param {string} field
   * @return {[unknown, unknown] | null} [<old>, <new>] | null
   */
  changedAttr(identifier: StableRecordIdentifier, field: string): [Value | undefined, Value] | null;
  ...
  /**
   * Tell the cache to discard any uncommitted mutations to a specific
   * attribute
   *
   * This method is a candidate to become a mutation
   *
   ...
   * @public
   * @param identifier
   * @return {boolean} whether the field was restored
   */
  rollbackAttr(identifier: StableRecordIdentifier, field: string): boolean;
  ...
  /**
   * Query the cache for the changes to a specific relationship of a resource.
   *
   * Returns a RelationshipDiff object.
   *
   * ```ts
   * type RelationshipDiff =
  | {
   ...
   */
  changedRelationship(identifier: StableRecordIdentifier, field: string): RelationshipDiff;
  ...
  /**
   * Tell the cache to discard any uncommitted mutations to a specific relationship.
   *
   * This will also discard the change on any appropriate inverses.
   *
   * This method is a candidate to become a mutation
   *
   * @method rollbackRelationship
   * @public
   * @param {StableRecordIdentifier} identifier
   * @param {field} string
   * @return {boolean} whether the relationship was restored
   */
  rollbackRelationship(identifier: StableRecordIdentifier, field: string): boolean;
  ...
  /**
   * Query the cache for the current (remote/non-mutated state of a relationship property
   *
   * @method getRelationship
   * @public
   * @param {StableRecordIdentifier} identifier
   ...
   * @return resource relationship object
   */
  peekRelationship(identifier: StableRecordIdentifier, field: string): ResourceRelationship | CollectionRelationship;

  /**
   * Query the cache for the current (mutated) state of a relationship property
   *
   * @method getRelationship
   * @public
   * @param {StableRecordIdentifier} identifier
 ...
 * data.
 *
 * Currently caches should implement V3 of the Cache interface,
 * while V2 is still supported for compatibility.
 *
 * @class <Interface> Cache
 * @public
 */
export type Cache = CacheV2 | CacheV3;
