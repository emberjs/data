...
DS.NestedRESTAdapter = DS.Adapter.extend({
  ...
  createRecord: function(store, type, record) {
    ...
    var url = this.urlForType(type, record) || root;
  ...
  createRecords: function(store, type, records) {
    ...
    // TODO is there a way to avoid records[0]?
    var url = this.urlForType(type, records[0]) || root;
  ...
  find: function(store, type, id) {
    var root = this.rootForType(type);
    // TODO should consider, if the assosiation is embeded!?
  ...
  urlForType: function(type, record) {
    var matches, parentRecordId = 0;
    if (type.url) { 
      matches = type.url.match(/%@([0-9]+)?/g);
      if (matches && record) {
        var parts = type.url.split('/');
        parentRecordId = record.get(this.singularize(parts[0])).get('id');
        
      }
      return Ember.String.fmt(type.url, [parentRecordId.toString()]);
    }
