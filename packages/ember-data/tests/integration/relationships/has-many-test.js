...
test("maintainedRecords - returns a filtered subset", function () {
  ...
  run(function() {
    var undeletedChapters = chapter.get('pages').maintainedRecords();
    equal(undeletedChapters.get('length'), 2);
    env.store.peekRecord('page', 3).deleteRecord();
    equal(undeletedChapters.get('length'), 1);
    env.store.peekRecord('page', 2).deleteRecord();
    equal(undeletedChapters.get('length'), 0);
  });
});
