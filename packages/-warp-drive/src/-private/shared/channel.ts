...
export function npmDistTagForChannelAndVersion(channel: CHANNEL, package_version: SEMVER_VERSION): NPM_DIST_TAG {
  ...
  switch (channel) {
    ...
    default:
      throw new Error(
        `Unable to determine npm dist-tag for channel ${channel as string} and version ${package_version}`
      );
  }
}
...
export function branchForChannelAndVersion(channel: CHANNEL, package_version: SEMVER_VERSION): VALID_BRANCHES {
  ...
  switch (channel) {
    ...
    default:
      throw new Error(
        `Unable to determine expected branch name for channel ${channel as string} and version ${package_version}`
      );
  }
}
