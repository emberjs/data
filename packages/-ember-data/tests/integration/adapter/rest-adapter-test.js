...
module('integration/adapter/rest_adapter - REST Adapter', function(hooks) {
  ...
  module('with setup adapter', function(hooks) {
    hooks.beforeEach(() => {
  ...
  module('init adapter coalesceFindRequests', function() {
    ...
    test('class findMany - passes buildURL the requestType', function(assert) {
      this.owner.unregister('adapter:application');
      this.owner.register('adapter:application', class MyClass extends RESTAdapter { coalesceFindRequests = true; });
      adapter = store.adapterFor('application');
