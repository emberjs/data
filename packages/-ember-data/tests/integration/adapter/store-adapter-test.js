...
module('integration/adapter/store-adapter - DS.Store and DS.Adapter integration test', function(hooks) {
  ...
  test("store.findRecord should pass 'fields' to adapter.findRecord", function(assert) {
    ...
    adapter.findRecord = (store, type, id, snapshot) => {
      assert.deepEqual(snapshot.adapterOptions.fields, { person: 'name' }, 'fields passed to adapter.findRecord');
      return resolve({ data: { id: 1, type: 'person' } });
    };

    run(() => store.findRecord('person', 1, { adapterOptions: { fields: { person: 'name' } } }));
  });
  ...
  test("store.findAll should pass 'fields' to adapter.findAll", function(assert) {
    ...
    adapter.findAll = function(store, type, sinceToken, arraySnapshot) {
      assert.deepEqual(arraySnapshot.adapterOptions.fields, { person: 'name' }, 'fields passed to adapter.findAll');
      return resolve({ data: [{ id: 1, type: 'person' }] });
    };

    run(() => store.findAll('person', { adapterOptions: { fields: { person: 'name' } } }));
  });
