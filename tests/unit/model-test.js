...
test('it should use `_internalModel` and not `internalModel` to store its internalModel', function(assert) {
  run(() => {
    ...
    assert.equal(
      store.peekRecord('person', 1).get('internalModel'),
      undefined,
      `doesn't shadow internalModel key`
    );
  });
});
...
testInDebug('Calling attr() throws a warning', function(assert) {
  ...
  assert.throws(
    ...
    /The `attr` method is not available on DS.Model, a DS.Snapshot was probably expected/,
    'attr() throws a warning'
  );
});
...
testInDebug('A subclass of DS.Model throws an error when calling create() directly', function(
  assert
) {
  assert.throws(
    () => {
