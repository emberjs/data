...
test('[ASSERTS KNOWN LIMITATION STILL EXISTS] returning new hasMany relationship info from a delete clears local state', function(assert) {
  ...
  env.serializer.normalize = function(klass, payload) {
    ...
    let normalized = _super.call(this, klass, payload);
    if (payload.data === null) {
      normalized.data = null;
    }
    normalized.included = payload.included;
    return normalized;
  };
  ...
  return run(() => {
    return shen.destroyRecord({})
      .then(() => {
        ...
        assert.deepEqual(pets.map(p => get(p, 'id')), ['2'], 'relationship now has only one pet, as the server overwrote the UI state');
      });
  });
});
