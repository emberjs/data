...
vi.mock('../../../../../packages/codemods/src/schema-migration/utils/config.js', async () => {
  const actual = await vi.importActual('../../../../../packages/codemods/src/schema-migration/utils/config.js');
  return {
    ...actual,
...
describe('generateConfig', () => {
  ...
  it('should handle inquirer cancellation', async () => {
    ...
    vi.mocked(generateConfig).mockRejectedValueOnce(new Error('User force closed the prompt with 0 null'));
  ...
  it('should handle other inquirer errors', async () => {
    ...
    vi.mocked(generateConfig).mockRejectedValueOnce(new Error('Some other error'));
