  ...
  run(function() {
    post.get('comments').then(function(comments) {
      ...
      post.get('comments').then(function(newComments) {
        ...
        assert.equal(
          newComments.objectAt(0).get('body'),
          'Third',
...
testInDebug(
  ...
  function(assert) {
    ...
    run(function() {
      all([env.store.findRecord('post', 1), env.store.findRecord('post', 2)]).then(function(
        records
      ) {
        assert.expectAssertion(function() {
    ...
    env.store
      ...
      .then(function(comment) {
        ...
        assert.equal(
          postCommentsLength,
          2,
          "comment's post should have a internalModel back to comment"
        );
        assert.ok(
...
test('deleteRecord + unloadRecord fun', function(assert) {
  ...
  run(() => {
    ...
    assert.deepEqual(posts.map(x => x.get('id')), [
      'post-1',
      'post-2',
      'post-3',
      'post-4',
...
test('A polymorphic hasMany should not be reordered when a child is posted with the reverse relationship specified', function (assert) {
  ...
  Message.reopen({
    post: DS.belongsTo('post', { async: true, inverse: 'messages' }),
  });
  ...
  env.registry.register(
    ...
    DS.RESTAdapter.extend({
      updateRecord(record) {
        return resolve({ comments: [commentData] });
      },
      createRecord() {
        return resolve({ comments: { id: commentId++ } });
      },
  ...
  let postData = {
    ...
    relationships: {
      messages: {
        data: [{ type: 'comment', id: '0' }, { type: 'comment', id: '1' }],
      },
    },
  };
  ...
  env.registry.register(
    ...
    DS.RESTAdapter.extend({
      findRecord(record) {
        return resolve({ posts: [postData] });
      },
      updateRecord(record) {
        return resolve({ posts: [postData] });
      },
      createRecord() {
        return resolve({ posts: { id: 1 } });
      },
    })
  );

  return run(() => {
    let comment1 = env.store.createRecord('comment');
    let comment2 = env.store.createRecord('comment');
    let post = env.store.createRecord('post', { id: 1, messages: [comment1, comment2] });
    return post
      .save()
      .then(() => comment1.save())
      .then(() => comment2.save())
      .then(() => post.get('messages'))
      .then(messages => {
        assert.equal(messages.toArray()[0], comment1, 'initially in the correct order');
      })
      .then(() => {
