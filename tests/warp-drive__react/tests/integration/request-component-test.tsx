...
import type {  ReactTestContext } from '@warp-drive/diagnostic/react';
import { module, setupTest, test as _test } from '@warp-drive/diagnostic/react';
...
interface LocalTestContext extends  ReactTestContext{
  ...
  test('it renders each stage of a request that succeeds', async function (assert) {
    ...
    await this.render(
        <Request
          request={request}
          states={{
            loading: () => <>Pending<br />Count: {countFor(request)}</>,
            error: ({ error }) => <>Error: {error.message}<br />Count: {countFor(error)}</>,
            content: ({ result }) => <>Content: {result.data.attributes.name}<br />Count: {countFor(result)}</>,
          }}
        />
  ...
  test('it renders only once when the promise already has a result cached', async function (assert) {
    ...
    await this.render(
      <Request
        ...
        states={{
          loading: () => <>Pending<br />Count: {countFor(request)}</>,
          cancelled: ({ error }) => <>Cancelled {error.message}<br />Count: {countFor(error)}</>,
          error: ({ error }) => <>Error: {error.message}<br />Count: {countFor(error)}</>,
          content: ({ result }) => <>Content: {result.data.attributes.name}<br />Count: {countFor(result)}</>,
        }}
      />
  ...
  test('we can retry from error state', async function (assert) {
    ...
    await this.render(
        <Request request={request}
          states={{
            loading: () => <>Pending<br />Count: {countFor(request)}</>,
            error: ({ error, features }) => <>{error.message}<br />Count: {countFor(error)}<button onClick={() => retry(features)} test-id="retry-button">Retry</button></>,
            content: ({ result }) => <>{result.data.attributes.name}<br />Count: {countFor(result)}</>,
          }}
        />
    );
