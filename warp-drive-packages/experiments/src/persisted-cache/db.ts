...
export async function getCachedRequest(
  ...
  const document: StructuredDocument<ResourceDocument<ExistingResourceObject & ExistingRecordIdentifier>> | null =
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve(
          request.result as unknown as StructuredDocument<
            ResourceDocument<ExistingResourceObject & ExistingRecordIdentifier>
          > | null
        );
      };
    });
...
function getCachedResource(
  ...
  const request = resourceStore.get(identifier.lid) as IDBRequest<ExistingRecordIdentifier>;
