...
/**
 * A React hook that allows you to run an object's
 * cleanup on component dismount.
 *
 * @param disposable A disposable object that has a `DISPOSE` method.
 */
export function useDisposable(disposable: { [DISPOSE]: () => void } | null): void {
  const initialized = useRef<null | { disposable: { [DISPOSE]: () => void } | null; dispose: () => void }>(null);

  useEffect(() => {
    if (!initialized.current || initialized.current.disposable !== disposable) {
      initialized.current = {
        disposable,
        dispose: () => {
          disposable?.[DISPOSE]();
    ...
    return initialized.current.dispose;
  }, [disposable]);
}
...
export function Request<RT, E>($props: RequestProps<RT, E>): JSX.Element {
  ...
  return (
    <ReactiveContext>
      <Chrome state={state.isIdle ? state.reqState : null} features={state.contentFeatures}>
        {
          // prettier-ignore
