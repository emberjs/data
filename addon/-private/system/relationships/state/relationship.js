...
export default class Relationship {
  constructor(store, inverseKey, relationshipMeta, modelData) {
  ...
  get parentType() {
    return this._tempModelName;
  }

  _inverseIsAsync() {
    if (!this.inverseKey || !this.inverseModelData) {
      return false;
    }
    // TODO IGOR DAVID this seems suspect, make sure its running
    return this.inverseModelData._relationships.get(this.inverseKey).isAsync;
  }
  ...
  addModelData(modelData, idx) {
    ...
    if (!this.members.has(modelData)) {
      ...
      if (this.inverseKey) {
        ...
        if (!modelData._implicitRelationships[this.inverseKeyForImplicit]) {
          modelData._implicitRelationships[this.inverseKeyForImplicit] = new Relationship(this.store, this.key,  { options: { async: this.isAsync } }, modelData);
        }
