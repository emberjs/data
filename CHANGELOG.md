...
* Add a doc heading for the ActiveModelSerializer class to so its page shows up in the API docs
...
* Add a base fiddle for mocked data requests with Ember Data
...
* Remove incorrect semicolon from pathForType example. Also changed the example to use .extend instead of reopen
* incase jQuery.ajax returns a null or undefined jqXHR
* Add `license` field to package.json
* Add `repository` field to package.json
* Cleanup scripts
* Fix to update ember-data version
* Use `package.json` as gem version
* Use npm-scripts to run test
* Specify language as Node.js on Travis CI
...
* Fix validation comments for
* Update changelog for initializer and container setup changes
* add bower to getting ember data section
...
* Fix a typo, accept -> except
* [doc] camelize `all` example for consistency
...
* actually update version for post-release version bump
