  ...
  check: function(file, errors) {
    ...
    file.iterateNodesByType(['FunctionDeclaration'], function(node) {
      ...
      if (beforeOpeningRoundBrace) {
        if (nextToken) {
          errors.add('Missing space before opening round brace', nextToken.loc.start);
        }
      } else {
        if (!nextToken) {
          errors.add('Illegal space before opening round brace', functionToken.loc.end);
        }
      }
      ...
      if (beforeOpeningCurlyBrace) {
        if (tokenBeforeBody) {
          errors.add('Missing space before opening curly brace', tokenBeforeBody.loc.start);
        }
      } else {
        if (!tokenBeforeBody) {
          errors.add('Illegal space before opening curly brace', node.body.loc.end);
        }
      }
    });
  },
