...
module.exports.prototype = {
  ...
  check: function(file, errors) {
    file.iterateNodesByType('CallExpression', function(node) {
      ...
      var lastArg =
        nodeBeforeRoundBrace.parentNode.arguments[
          nodeBeforeRoundBrace.parentNode.arguments.length - 1
        ];
      ...
      if (spaceAfterOpeningRoundBraceExists) {
        errors.add('Illegal space after opening round brace', roundBraceTokenStart.loc.start);
      }

      if (spaceBeforeClosingRoundBraceExists) {
        errors.add('Illegal space before closing round brace', roundBraceTokenEnd.loc.start);
      }
    });
  },
};
